...
class NewRequest extends React.Component {
    ...
    handleChangedActiveDonor(event) {
        if (event.target.id==='activeDonorYes' && event.target.checked) {
            this.setState({activeDonor: 'yes'});
        } else {
            this.setState({activeDonor: 'no'});
        }
    }
    ...
    handleSubmit(event) {
        ...
        if (ok) {
            let toSend = {
                ...
                idDonationCenter: this.state.idDonationCenter,
                ...
                <div className="container">
                    <div className="row above-footer align-items-center justify-content-center">
                        <div className="col-11" style={{textAlign: 'center'}}>
                            <h1 className="questrial-font" style={{marginBottom: '90px', marginTop: '100px', fontSize: '40px'}}>New Request Form</h1>
                        </div>
                        <div className="col-11 col-sm-8 col-md-6 col-lg-5" style={{textAlign: 'center'}}>

                            {/* Blood Group */}
                            <p style={{fontWeight: 'lighter', fontSize: '18px', textAlign: 'left', marginBottom: '5px'}}>Blood group</p>
                            ...
                            <Form handleSubmit={this.handleSubmit} onChange={this.handleChangedTextField}>
                                ...
                                <Input name="idDonationCenter" label='ID of the donation center which will receive the request (available at "City Blood Stocks")' type="text"/>
                            ...
                            {/* Urgency level */}
                            <p style={{fontWeight: 'lighter', fontSize: '18px', textAlign: 'left', paddingTop: '25px', marginBottom: '5px'}}>Urgency level</p>
                            <div className="form-check" style={{textAlign: 'left', marginLeft: '20px', fontWeight: 'lighter'}}>
                                <input className="form-check-input" type="radio" name="urgencyOptions" id="urgencyHigh" value="high" ref="urgencyHigh" onChange={this.handleChangedUrgency}/>
                                <label className="form-check-label">High</label>
                            </div>
                            <div className="form-check" style={{textAlign: 'left', marginLeft: '20px', fontWeight: 'lighter'}}>
                                <input className="form-check-input" type="radio" name="urgencyOptions" id="urgencyMedium" value="medium" ref="urgencyMedium" onChange={this.handleChangedUrgency}/>
                                <label className="form-check-label">Medium</label>
                            </div>
                            <div className="form-check" style={{textAlign: 'left', marginLeft: '20px', fontWeight: 'lighter'}}>
                                <input className="form-check-input" type="radio" name="urgencyOptions" id="urgencyLow" value="low" ref="urgencyLow" onChange={this.handleChangedUrgency}/>
                                <label className="form-check-label">Low</label>
                            </div>

                            {/* Active Donor */}
                            <p style={{fontWeight: 'lighter', fontSize: '18px', textAlign: 'left', paddingTop: '15px', marginBottom: '5px'}}>Is the beneficiary an active donor or not?</p>
                            <div className="form-check" style={{textAlign: 'left', marginLeft: '20px', fontWeight: 'lighter'}}>
                                <input className="form-check-input" type="radio" name="activeDonorRadioOptions" id="activeDonorYes" value="yes" ref="activeDonorYes" onChange={this.handleChangedActiveDonor}/>
                                <label className="form-check-label">Yes</label>
                            </div>
                            <div className="form-check" style={{textAlign: 'left', marginLeft: '20px', marginTop: '-7px', fontWeight: 'lighter'}}>
                                <input className="form-check-input" type="radio" name="activeDonorRadioOptions" id="activeDonorNo" value="no" ref="activeDonorNo" onChange={this.handleChangedActiveDonor}/>
                            ...
                            <div style={{marginTop: '90px', marginBottom: '100px'}}>
                                ...
                                <BlockButton onClick={this.handleSubmit} color="#ec0a0b" width="220px" fontFamily="Questrial" disabled={this.state.disabledSubmitButton}>SEND REQUEST</BlockButton>
                            </div>
                        </div>
                    </div>
                </div>
